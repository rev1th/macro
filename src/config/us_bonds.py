...
def construct():
    ...
    for _, b_r in bond_prices.iterrows():
        mat_date = b_r['MATURITY_DATE'].date()
        if b_r['TYPE'] == 'BILL':
            if mat_date < min_maturity_bill:
                continue
            bill_obj = Bill(mat_date, name=b_r['CUSIP'], _settle_delay=date_lib.Tenor.bday(1))
            ...
            if mat_date < min_maturity_bond:
                continue
            bond_obj = Bond(mat_date, b_r['RATE'], date_lib.Frequency.SemiAnnual,
